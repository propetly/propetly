...
from .serializers import AgencySerializer, CreateAgencySerializer


class AgencyList(APIView):
    def get(self, request, format=None):
        agencies = Agency.objects.order_by("id").all()
...
class AgencyDetail(APIView):
    ...
    def put(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = CreateAgencySerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
